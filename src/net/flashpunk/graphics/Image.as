package net.flashpunk.graphics 
{
	...
	public class Image extends Graphic
	{
		...
		/**
		 * Creates a new rectangle Image filled with texture.
		 * @param	width		Width of the rectangle.
		 * @param	height		Height of the rectangle.
		 * @param	texture		BitmapData texture used to fill the rectangle.
		 * @param	alpha		Image alpha.
		 * @return	A new Image object.
		 */
		public static function createTextureRect(width:uint, height:uint, texture:BitmapData, alpha:Number = 1):Image 
		{
			var image:Image;
			
			// create a new BitmapData that will hold our rect filled with texture
			var bmData:BitmapData = new BitmapData(width, height, true, 0x0);
			
			// fill our BitmapData with texture
			var _graphics:Graphics = FP.sprite.graphics;
			_graphics.clear();
			_graphics.beginBitmapFill(texture);
			_graphics.drawRect(0, 0, width, height);
			_graphics.endFill();
			bmData.draw(FP.sprite);
			
			// create a new Image with our BitMapData
			image = new Image(bmData);
			image.alpha = alpha;
